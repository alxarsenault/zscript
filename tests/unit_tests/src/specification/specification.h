...
{
  std::stringstream ss;
  ss << section_name<Section>() << "-" << std::setfill('0') << std::setw(3)
     << get_section_count<Section>();
...
{
  inline z_code_test_fixture(section sec,
    std::string_view ccode,
    std::string_view ccode_extra,
    ...
    , code(ccode)
  {
    ...
    vm.get_engine()->add_import_directory(
      ZSCRIPT_STANDARD_TESTS_RESOURCES_DIRECTORY);
    ...
    //
    if (auto err
      = vm->call(closure, n_params, vm.stack_size() - n_params, value)) {
      error = err;
...
#define ZS_SPEC_TEST_INTERNAL_CATCH_TEST_CASE_METHOD2(TestName,                \
  ClassName,                                                                   \
  code,                                                                        \
  code_extra,                                                                  \
  sec,                                                                         \
  ...
  {                                                                            \
    ...
    const Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(            \
      ...
      #ClassName##_catch_sr,                                                   \
      Catch::NameAndTags{ TestName::get_test_name() });                        \
  }                                                                            \
  CATCH_INTERNAL_STOP_WARNINGS_SUPPRESSION                                     \
  void TestName::test()

#define ZS_SPEC_TEST_5(sec, title, desc, code, code_extra)                     \
  ZS_SPEC_TEST_INTERNAL_CATCH_TEST_CASE_METHOD2(                               \
    INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEST_),                         \
    zspec::z_code_test_fixture,                                                \
    code,                                                                      \
    code_extra,                                                                \
    ZSECTION(sec),                                                             \
    title,                                                                     \
...
#define ZS_SPEC_TEST_7(                                                        \
  sec, title, desc, code, code_extra, require_compile, require_call)           \
  ZS_SPEC_TEST_INTERNAL_CATCH_TEST_CASE_METHOD2(                               \
    INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEST_),                         \
    zspec::z_code_test_fixture,                                                \
    code,                                                                      \
...
#define ZS_SPEC_TEST(...)                                                      \
  ZBASE_DEFER(ZBASE_CONCAT(ZS_SPEC_TEST_, ZBASE_NARG(__VA_ARGS__)), __VA_ARGS__)
...
#define ZS_SPEC_SUBSECTION(sec, name, description)                             \
  ...
  {                                                                            \
    zb::stream_print<"">(zspec::get_stream(),                                  \
      ...
      ((int)ZSECTION(sec) + 1),                                                \
      ".",                                                                     \
      zspec::begin_subsection<ZSECTION(sec)>(),                                \
      ". ",                                                                    \
      name,                                                                    \
      "\n\n",                                                                  \
      description,                                                             \
