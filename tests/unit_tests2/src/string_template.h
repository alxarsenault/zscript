...
struct string_template_traits {
  ...
  constexpr static inline std::basic_string_view<char_t> default_arg_regex
      = detail::default_arg_regex<char_t>();
...
public:
  ...
  basic_string_template(const typename args_map_t::allocator_type& a_alloc,
      const typename parts_vector_t::allocator_type& p_alloc)
      : m_args(a_alloc)
      , m_parts(p_alloc) {}
  ...
  void parse_template(string_view_t str_template) {
    ...
    parse_template(
        str_template, regex_t(default_arg_regex.data(), default_arg_regex.length()), match_results);
  }
  ...
  void parse_template(string_view_t str_template, string_view_t arg_regex,
      const typename match_results_t::allocator_type& alloc) {
  ...
  bool set_arg(string_view_t key, arg_value_t value) {
    if (auto it = m_args.find(key); it != m_args.end()) {
      it->second.template emplace<1>(std::move(value));
      return true;
    }
  ...
  bool emplace_arg(string_view_t key, Args&&... args) {
    if (auto it = m_args.find(key); it != m_args.end()) {
      it->second.template emplace<1>(std::forward<Args>(args)...);
      return true;
    }
...
template <class StringTemplate = string_template>
StringTemplate make_template(
    typename StringTemplate::string_view_t str_template, typename StringTemplate::string_view_t arg_regex) {
  StringTemplate st;
...
StringTemplate make_template(typename StringTemplate::string_view_t str_template,
    typename StringTemplate::string_view_t arg_regex, const Alloc& alloc) {
...
template <class StringTemplate, class Alloc>
StringTemplate make_template(
    typename StringTemplate::string_view_t str_template, default_arg_regex_t, const Alloc& alloc) {
  StringTemplate st(alloc);
