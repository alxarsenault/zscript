...
ZB_INLINE constexpr bool has_simd_opt() {
#if __ZBASE_HAS_SIMD__
...
template <typename UC>
ZB_INLINE constexpr bool is_integer(UC c) noexcept {
  return !(c > UC('9') || c < UC('0'));
}

ZB_INLINE constexpr uint64_t byteswap(uint64_t val) {
  return (val & 0xFF00000000000000) >> 56 | (val & 0x00FF000000000000) >> 40
      | (val & 0x0000FF0000000000) >> 24 | (val & 0x000000FF00000000) >> 8 | (val & 0x00000000FF000000) << 8
      | (val & 0x0000000000FF0000) << 24 | (val & 0x000000000000FF00) << 40
      | (val & 0x00000000000000FF) << 56;
}

// Read 8 UC into a u64. Truncates UC if not char.
template <typename UC>
ZB_INLINE constexpr uint64_t read8_to_u64(const UC* chars) {
...
ZB_INLINE uint64_t simd_read8_to_u64(const __m128i data) {
  ZBASE_PRAGMA_PUSH()
  ZBASE_PRAGMA_DISABLE_WARNING_GCC("-Wcast-align")
...
ZB_INLINE uint64_t simd_read8_to_u64(const char16_t* chars) {
  ZBASE_PRAGMA_PUSH()
  ZBASE_PRAGMA_DISABLE_WARNING_GCC("-Wcast-align")
...
ZB_INLINE uint64_t simd_read8_to_u64(const uint16x8_t data) {
  ZBASE_PRAGMA_PUSH()
  ZBASE_PRAGMA_DISABLE_WARNING_GCC("-Wcast-align")
...
#endif // __ZBASE_SSE2__

// MSVC SFINAE is broken pre-VS2017
#if defined(_MSC_VER) && _MSC_VER <= 1900
template <typename UC>
#else
template <typename UC, std::enable_if_t<!has_simd_opt<UC>(), int> = 0>
#endif
// dummy for compile
...
// credit  @aqrit
ZB_INLINE constexpr uint32_t parse_eight_digits_unrolled(uint64_t val) {
...
template <typename UC>
ZB_INLINE constexpr uint32_t parse_eight_digits_unrolled(UC const* chars) noexcept {
...
// credit @aqrit
ZB_INLINE constexpr bool is_made_of_eight_digits_fast(uint64_t val) noexcept {
...
ZB_INLINE constexpr bool simd_parse_if_eight_digits_unrolled(const char16_t* chars, uint64_t& i) noexcept {
...
template <typename UC, std::enable_if_t<!std::is_same<UC, char>::value, int> = 0>
ZB_INLINE constexpr void loop_parse_if_eight_digits(const UC*& p, const UC* const pend, uint64_t& i) {
  if (!has_simd_opt<UC>()) {
    return;
  }
...
ZB_INLINE constexpr void loop_parse_if_eight_digits(const char*& p, const char* const pend, uint64_t& i) {
...
template <typename UC>
ZB_INLINE constexpr parsed_number_string_t<UC> report_parse_error(UC const* p, parse_error error) {
...
template <typename UC>
ZB_INLINE constexpr parsed_number_string_t<UC> parse_number_string(
...
ZB_INLINE constexpr from_chars_result_t<UC> parse_int_string(
  ...
  if (!std::is_signed<T>::value && negative) {
    //    answer.ec = __zb::errc::invalid_argument;
    //    answer.ptr = first;
    return { __zb::errc::invalid_argument, first };
  }
#ifdef FASTFLOAT_ALLOWS_LEADING_PLUS // disabled by default
  if ((*p == UC('-')) || (*p == UC('+'))) {
#else
  ...
  if (digit_count == 0) {
    if (has_leading_zeros) {
      value = 0;
      answer.set_error(__zb::errc::success);
      answer.set_value(p);
    }
    else {
      answer.set_error(__zb::errc::invalid_argument);
      answer.set_value(first);
    }
    return answer;
  }
  ...
  if (digit_count > max_digits) {
    answer.set_error(__zb::errc::result_out_of_range);
    return answer;
  }
