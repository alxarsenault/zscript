...
public:
  ...
  ZS_INLINE zs::error_result add_import_directory(zb::string_view directory) {
    return add_import_directory(std::filesystem::path(std::string_view(directory)));
  }

  ZS_INLINE zs::error_result add_import_directory(const char* directory) {
    return add_import_directory(std::filesystem::path(directory));
  }
