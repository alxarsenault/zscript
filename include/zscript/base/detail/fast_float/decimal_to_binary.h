...
ZB_INLINE constexpr value128 compute_product_approximation(int64_t q, uint64_t w) {
 ...
 *   p = log(5**-q)/log(2) = -q * log(5)/log(2)
 */
constexpr ZB_INLINE int32_t power(int32_t q) noexcept { return (((152170 + 65536) * q) >> 16) + 63; }
} // namespace detail

// create an adjusted mantissa, biased by the invalid power2
// for significant digits already multiplied by 10 ** q.
template <typename binary>
ZB_INLINE constexpr adjusted_mantissa compute_error_scaled(int64_t q, uint64_t w, int lz) noexcept {
  int hilz = int(w >> 63) ^ 1;
...
template <typename binary>
ZB_INLINE constexpr adjusted_mantissa compute_error(int64_t q, uint64_t w) noexcept {
...
template <typename binary>
ZB_INLINE constexpr adjusted_mantissa compute_float(int64_t q, uint64_t w) noexcept {
  adjusted_mantissa answer;
