...
struct internal {
  ...
  ZB_INLINE static T* zs_new(zs::engine* eng, Args&&... args) {
  ...
  template <class T>
  ZB_INLINE static void zs_delete(zs::engine* eng, T* ptr) {
    ptr->~T();
    zs::deallocate(eng, ptr);
  }

  template <class T>
  ZB_INLINE static void zs_delete(zs::engine* eng, T* ptr, memory_tag tag) {
    ptr->~T();
...
template <memory_tag Tag, class T, class... Args>
ZB_INLINE T* zs_new(zs::engine* eng, Args&&... args) {
  return internal::zs_new<Tag, T, Args...>(eng, std::forward<Args>(args)...);
}

template <class T>
ZB_INLINE void zs_delete(zs::engine* eng, T* ptr) {
...
public:
  ...
  ZB_CK_INLINE size_t ref_count() const noexcept { return _ref_count; }
...
struct object_table_hash {
  ...
  ZS_CK_INLINE size_t operator()(const char* s) const noexcept;
};
