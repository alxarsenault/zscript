...
static inline int to_chars(const floating_decimal_64 v, const bool sign, char* const result) {
  ...
  if ((output >> 32) != 0) {
    ...
    memcpy(result + index + olength - 1, __zb::k_digit_table.data() + c0, 2);
    memcpy(result + index + olength - 3, __zb::k_digit_table.data() + c1, 2);
    memcpy(result + index + olength - 5, __zb::k_digit_table.data() + d0, 2);
    memcpy(result + index + olength - 7, __zb::k_digit_table.data() + d1, 2);
    i += 8;
  }
  ...
  while (output2 >= 10000) {
    ...
    const uint32_t c0 = (c % 100) << 1;
    const uint32_t c1 = (c / 100) << 1;
    memcpy(result + index + olength - i - 1, __zb::k_digit_table.data() + c0, 2);
    memcpy(result + index + olength - i - 3, __zb::k_digit_table.data() + c1, 2);
    i += 4;
  }
  if (output2 >= 100) {
    const uint32_t c = (output2 % 100) << 1;
    output2 /= 100;
    memcpy(result + index + olength - i - 1, __zb::k_digit_table.data() + c, 2);
    i += 2;
  }
  if (output2 >= 10) {
    const uint32_t c = output2 << 1;
    // We can't use memcpy here: the decimal dot goes between these two digits.
    result[index + olength - i] = __zb::k_digit_table[c + 1];
    result[index] = __zb::k_digit_table[c];
  }
  else {
...
static inline bool d2d_small_int(
    const uint64_t ieeeMantissa, const uint32_t ieeeExponent, floating_decimal_64* const v) {
