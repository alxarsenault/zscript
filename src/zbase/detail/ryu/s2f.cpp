...
static inline uint32_t floor_log2(const uint32_t value) { return 31 - __builtin_clz(value); }
...
enum Status s2f_n(const char* buffer, size_t len, float* result) {
  ...
  uint32_t ieee
      = (((((uint32_t)signedM) << FLOAT_EXPONENT_BITS) | (uint32_t)ieee_e2) << FLOAT_MANTISSA_BITS) | ieee_m2;
  *result = int32Bits2Float(ieee);
  return SUCCESS;
}
