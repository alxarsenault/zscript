...
inline constexpr char alt_directory_separator = '/';
#else
inline constexpr char directory_separator = '/';
inline constexpr char alt_directory_separator = '\\';
#endif
...
static size_t write_callback(void* opaque, std::uint64_t file_ofs, const void* pBuf, size_t n) {
...
zip_file::zip_file(const std::string& filename)
    : zip_file() {
  load(filename);
}

zip_file::zip_file(std::istream& stream)
    : zip_file() {
  load(stream);
}

zip_file::zip_file(const std::vector<uint8_t>& bytes)
    : zip_file() {
  load(bytes);
}

zip_file::~zip_file() { reset(); }
...
void zip_file::load(std::span<const uint8_t> bytes) {
  reset();
...
void zip_file::save(std::vector<uint8_t>& bytes) {
  if (_archive->m_zip_mode == MZ_ZIP_MODE_WRITING) {
    mz_zip_writer_finalize_archive(_archive.get());
  }
...
bool zip_file::has_file(const std::string& name) const {
  if (_archive->m_zip_mode != MZ_ZIP_MODE_READING) {
    start_read();
  }

  int index = mz_zip_reader_locate_file(_archive.get(), name.c_str(), nullptr, 0);

  return index != -1;
}

bool zip_file::has_file(const zip_info& name) const { return has_file(name.filename); }

zip_info zip_file::getinfo(const std::string& name) const {
  if (_archive->m_zip_mode != MZ_ZIP_MODE_READING) {
    start_read();
  }
...
std::vector<zip_info> zip_file::infolist() const {
  if (_archive->m_zip_mode != MZ_ZIP_MODE_READING) {
    start_read();
  }
  ...
  for (size_t i = 0; i < mz_zip_reader_get_num_files(_archive.get()); i++) {
...
std::vector<std::string> zip_file::namelist() const {
...
std::ostream& zip_file::open(const std::string& name) { return open(getinfo(name)); }

std::ostream& zip_file::open(const zip_info& name) {
  auto data = read(name);
  std::string data_string(data.begin(), data.end());
  _open_stream << data_string;
  return _open_stream;
}
...
void zip_file::extractall(const std::string& path) { extractall(path, infolist()); }
...
void zip_file::printdir() const { printdir(std::cout); }

void zip_file::printdir(std::ostream& stream) const {
  stream << "  Length "
         << "  "
         << "   "
         << "Date"
...
std::string zip_file::read(const zip_info& info) const {
  size_t size;
  char* data = static_cast<char*>(
      mz_zip_reader_extract_file_to_heap(_archive.get(), info.filename.c_str(), &size, 0));
  if (data == nullptr) {
...
std::vector<uint8_t> zip_file::read_buffer(const zip_info& info) const {
  size_t size;
  ...
  std::vector<uint8_t> extracted(data, data + size);
  mz_free(data);
  return extracted;
}

std::string zip_file::read(const std::string& name) const { return read(getinfo(name)); }
...
void zip_file::writestr(const std::string& arcname, const std::string& bytes) {
  ...
  if (!mz_zip_writer_add_mem(
          _archive.get(), arcname.c_str(), bytes.data(), bytes.size(), MZ_BEST_COMPRESSION)) {
...
void zip_file::writestr(const zip_info& info, const std::string& bytes) {
  ...
  if (!mz_zip_writer_add_mem_ex(_archive.get(), info.filename.c_str(), bytes.data(), bytes.size(),
          info.comment.c_str(), static_cast<mz_uint16>(info.comment.size()), MZ_BEST_COMPRESSION, 0, crc)) {
    throw std::runtime_error("write error");
  }
}

void zip_file::start_read() const {
  if (_archive->m_zip_mode == MZ_ZIP_MODE_READING)
    return;
...
void zip_file::start_write() {
  ...
  case MZ_ZIP_MODE_READING: {
    ...
    _archive->m_pWrite = &detail::write_callback;
...
void zip_file::append_comment() {
  if (!comment.empty()) {
    ...
    std::copy(comment.begin(), comment.end(), std::back_inserter(_buffer));
  }
}
...
void zip_file::remove_comment() {
  ...
  size_t position = _buffer.size() - 1;

  for (; position >= 3; position--) {
    if (_buffer[position - 3] == 'P' && _buffer[position - 2] == 'K' && _buffer[position - 1] == '\x05'
        && _buffer[position] == '\x06') {
  ...
  if (length != 0) {
    comment = std::string(_buffer.data() + position, _buffer.data() + position + length);
    _buffer.resize(_buffer.size() - length);
    _buffer[_buffer.size() - 1] = 0;
    _buffer[_buffer.size() - 2] = 0;
  }
}

zip_info zip_file::getinfo(int index) const {
  if (_archive->m_zip_mode != MZ_ZIP_MODE_READING) {
    start_read();
  }
