#include "lex/zpreprocessor_lexer.h"
...
struct preprocessor_lexer::helper {
  ...
  inline static void skip_hashtag_comment(preprocessor_lexer* l) {
    auto& s = l->_stream;
  ...
  inline static void skip_single_line_comment(preprocessor_lexer* l) {
    auto& s = l->_stream;
  ...
  inline static void skip_multi_line_comment(preprocessor_lexer* l) {
    auto& s = l->_stream;
  ...
  inline static void skip_block_comment(preprocessor_lexer* l) {
    auto& s = l->_stream;
  ...
  inline static void parse_identifier(preprocessor_lexer* l) {
  ...
  inline static void parse_string(preprocessor_lexer* l, uint32_t end_char) {
  ...
  inline static void parse_multi_line_string(preprocessor_lexer* l, uint32_t end_char) {
    zb::utf8_span_stream& s = l->_stream;
  ...
  static inline zs::error_result parse_number(preprocessor_lexer* l) {
    zb::utf8_span_stream& s = l->_stream;
  ...
  static inline void parse_operator(preprocessor_lexer* l);
};

preprocessor_lexer::preprocessor_lexer(zs::engine* eng)
    : engine_holder(eng)
    , _escaped_string(zs::allocator<char>(eng)) {}

preprocessor_lexer::preprocessor_lexer(zs::engine* eng, std::string_view code)
    : preprocessor_lexer(eng) {

  init(code);
}

preprocessor_lexer::~preprocessor_lexer() {}

void preprocessor_lexer::init(std::string_view code) {
...
zs::object preprocessor_lexer::get_value() const {
...
zs::object preprocessor_lexer::get_debug_value() const {
  switch (_current_token) {
  case tok_none:
    return zs::object::create_none();
  case tok_null:
...
namespace preprocessor_lexer_detail {
...
void preprocessor_lexer::helper::parse_operator(preprocessor_lexer* l) {
  zb::utf8_span_stream& s = l->_stream;
  ...
  case tok_identifier: {
    {
      using namespace preprocessor_lexer_detail;
...
token_type preprocessor_lexer::lex() {
...
zs::error_result preprocessor_lexer::lex_compare(std::span<const token_type> buffer) {
...
token_type preprocessor_lexer::peek() const {
  preprocessor_lexer l(*this);
  return l.lex();
}

zs::error_result preprocessor_lexer::peek(std::span<token_type>& buffer) const {
  preprocessor_lexer l(*this);
...
bool preprocessor_lexer::is_template_function_call() const {
...
zs::error_result preprocessor_lexer::lex(std::span<token_type>& buffer) {
  auto it = buffer.begin();
  const auto end = buffer.end();
...
zs::error_result preprocessor_lexer::lex(std::span<token_type>& buffer, token_type tok) {
  auto it = buffer.begin();
  const auto end = buffer.end();
...
zs::error_result preprocessor_lexer::lex_rbracket(std::span<token_type>& buffer) {
  auto it = buffer.begin();
  const auto end = buffer.end();
...
zs::error_result preprocessor_lexer::lex_for_auto(std::span<token_type>& buffer) {
  auto it = buffer.begin();
  const auto end = buffer.end();
...
zs::error_result preprocessor_lexer::lex_to(token_type tok) {
...
zs::error_result preprocessor_lexer::lex_to(token_type tok, size_t nmax) {
  size_t count = 0;
  while (!zb::is_one_of(_current_token, tok_eof, tok_lex_error)) {
