...
ZS_JIT_COMPILER_PARSE_OP(p_module_info, module_info_type mtype) {
  ...
  if (mtype == module_info_type::module) {
    if (!_ccs->is_top_level() or _is_header < 2) {
      return ZS_COMPILER_ERROR(invalid_module_tag,
          "The @module statement can only happen once in the top level scope, before any other "
          "statements.\n");
    }

    if (_ccs->is_module()) {
      return ZS_COMPILER_ERROR(duplicated_module_tag, "Duplicated @module tag.\n");
    }
  ...
  if (!_ccs->is_top_level() or !_ccs->is_module() or _is_header < 2) {
    return ZS_COMPILER_ERROR(invalid_module_info_tag, "The ", module_info_type_to_string(mtype),
  ...
  if (is(tok_endl)) {
    return ZS_COMPILER_ERROR(invalid_module_info_tag, "No end line is allowed be a ",
  ...
  if (content.get_string_unchecked().empty()) {
    return ZS_COMPILER_ERROR(
        invalid_module_info_tag, "The ", module_info_type_to_string(mtype), " statement requires a value.\n");
  }
