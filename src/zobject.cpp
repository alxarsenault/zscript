...
object object::create_mutable_string(zs::engine* eng, zs::string&& s) {
  object obj;
  obj._type = object_type::k_mutable_string;
  obj._mstring = mutable_string_object::create(eng, std::move(s));
  return obj;
}
...
bool object::is_double_question_mark_true() const noexcept {
  ...
  case k_mutable_string:
    return !get_mutable_string_unchecked().empty();
  case k_array:
...
zs::error_result object::set_delegate(object delegate) noexcept {
  ...
  as_delegate().set_delegate(delegate);
  return {};
  //
  //  switch (_type) {
  //  case k_array:
...
zs::error_result object::get_delegate(object& delegate) const noexcept {
  if (!is_delegable()) {
    return zs::errc::invalid_type;
  }

  if (as_delegate().has_delegate()) {
    delegate = as_delegate().get_delegate();
    return {};
  }
...
zs::object object::get_delegate() const noexcept {
  return is_delegable() ? as_delegate().get_delegate() : create_null();
}
