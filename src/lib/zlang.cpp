...
namespace {
  int_t zlang_import_impl(zs::vm_ref vm) {
    ...
    if (auto err = zs::import_module(vm, name, output_module)) {
      vm->set_error("Can't import module ", name, ".\n");
      return -1;
    }

    return vm.push(output_module);
    //  table_object& tbl = vm->get_imported_modules().as_table();
    //
    //  object dest;
  ...
  int_t zlang_create_array_impl(zs::vm_ref vm) {
    ...
    if (arg1.is_meta_argument()) {
      ...
      if (!meta_arr[0].has_flags(object_flags_t::f_array_type)) {
        return -1;
      }
  ...
  int_t zlang_toweak_impl(zs::vm_ref vm) { return vm.push(vm[1].get_weak_ref()); }
} // namespace.
...
#undef _X
  ...
  root[_ss("print")] = zs::parameter_list_function_t([](zs::vm_ref vm, parameter_list params) -> var {
    return zlang_print_internal(vm, params, vm, " ", "\n");
  });

  root.emplace(_ss("__tostring"), zlang_tostring_impl);
  root.emplace(_ss("__toint"), zlang_toint_impl);
  root.emplace(_ss("__tofloat"), zlang_tofloat_impl);
  root.emplace(_ss("toweak"), zlang_toweak_impl);

  root[_ss("dprint")] = _nf(zlang_dprint_impl);
  //  root[_ss("now")] = _nf(zlang_now_impl);

  //  root[_ss("optget")] = _nf(zlang_optget_impl);
  root[_ss("__create_array")] = _nf(zlang_create_array_impl);
...
#undef _X
    ...
    na_module_tbl[_ss("__create_array")] = +[](zs::vm_ref vm) -> int_t {
      if (vm.stack_size() == 3) {
    ...
    vm->get_imported_modules().as_table()["na"] = na_module;
  }
