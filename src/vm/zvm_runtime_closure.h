...
ZS_DECL_RT_ACTION(new_closure, uint32_t fct_idx, uint8_t bounded_target, objref_t dest) {
  ...
  const zs::object& new_closure_fct_proto = current_closure_fct_proto.as_proto()._functions[fct_idx];

  // TODO: Use closure's root.
  // Create the new closure object.
  object new_closure_obj = zs::_c(_engine, new_closure_fct_proto, _root_table);
  ...
  if (int_t module_idx = current_closure_fct_proto._fproto->_export_table_target; module_idx != -1) {
    ...
    new_closure._module = _stack[module_idx];
  }
    ...
    for (size_t i = 0; i < capture_sz; i++) {
      ...
      case captured_variable::local: {
        ...
        for (object& p : _open_captures) {
          if (p.as_capture().get_value_ptr() == ptr) {
            found_cap = &p;
            break;
            //            zb::print("dsdsdsdsds");
          }
        ...
        if (found_cap) {
          ...
          new_closure_captured_values.emplace_back(zs::capture_object::create(_engine, ptr), false);
      ...
      case captured_variable::outer: {
        ...
        ZS_ASSERT(current_closure_captured_values[cap_idx].is_capture());
