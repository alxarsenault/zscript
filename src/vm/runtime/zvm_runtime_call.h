...
ZS_DECL_RT_ACTION(
  ...
  const int_t n_expected_params = nclosure->get_parameters_count();

  // The closure has `n_default_params` default parameters values.
  const int_t n_default_params = nclosure->get_default_parameters_count();

  // A span of parameters that will be pushed after the `n_params`  if needed.
  ...
  if (n_default_params && n_params < n_expected_params) {
    ...
    if (diff <= n_default_params) {
      ...
      default_params = std::span<object>(&nclosure->_default_params[start_index], diff);
    }
  }
  ...
  object vargs;
  if (nclosure->_has_vargs_params and n_params >= n_expected_params) {
  ...
  //
  //    // The current top of the stack will be the next stack base.
  //    const size_t next_base = _stack.get_absolute_top();
  //
  //    // Push `n_params` elements starting at `stack_base` on top of the stack.
  //    _stack.absolute_repush_n(stack_base, n_params);
  //
  //    if (vargs.is_array()) {
  //      _stack.push(vargs);
  //      n_params = n_expected_params;
  ...
  //    _stack.set_stack_base(next_base);
  //
  //    zbase_assert(next_base == (_stack.get_absolute_top() - n_params), "invalid stack parameters");
  //    zbase_assert((int_t)_stack.stack_size() == n_params, "invalid stack parameters");
  //  }
  ...
  const int_t call_result = nclosure->call(zs::vm_ref(this));
  ...
  ret_value.get() = call_result ? _stack.top() : nullptr;
...
ZS_DECL_RT_ACTION(call_native_closure, cobjref_t closure_obj, zs::parameter_list params, objref_t ret_value) {
  ...
  // The call was made with `n_params` parameters.
  // The function expects `n_expected_params` parameters.
  const int_t n_expected_params = nclosure->_parameter_names.size();

  // The closure has `n_default_params` default parameters values.
  const int_t n_default_params = nclosure->_default_params.size();
  ...
  if (nclosure->_has_vargs_params and n_params >= n_expected_params) {
    ...
    for (int_t i = 0; i < nvargs; i++) {
      vargs.as_array()[i] = params[n_expected_params - 1 + i];
    }

    params = params.subspan(0, n_expected_params - 1);
    default_params = std::span<object>();
    n_params = n_expected_params - 1;
  }
  else if (n_expected_params != n_params + n_extra_default) {
