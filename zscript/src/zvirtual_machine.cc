...
#define ZS_DECL_OPCODE(name) &virtual_machine::exec_op_wrapper<ZS_OPCODE_ENUM_VALUE(name)>,
...
virtual_machine* virtual_machine::create(size_t stack_size, allocate_t alloc_cb, raw_pointer_t user_pointer,
  ...
  eng = zb_placement_new(eng) zs::engine(alloc_cb, user_pointer, user_release, stream_getter);
...
static void destroy_engine(zs::engine* eng) {
  ...
  eng->~engine();
  (*alloc_cb)(eng, user_ptr, eng, 0, sizeof(zs::engine), alloc_info_t{});

  // User release is called after deleting the engine.
  if (user_release) {
    (*user_release)(nullptr, user_ptr);
  }
}
...
zs::error_result virtual_machine::make_enum_table(object& obj) {
  ...
  obj._flags = object_flags_t::enum_table;
  return {};
}
