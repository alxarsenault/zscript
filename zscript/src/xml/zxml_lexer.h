...
class xml_lexer : public engine_holder {
  ZS_CLASS_COMMON;
...
public:
  ...
  xml_lexer(zs::engine* eng);
  ~xml_lexer();

  void init(std::string_view code);

  xml_token_type lex();
  xml_token_type lazy_lex();
  xml_token_type peek() const;
  ...
  inline xml_token_type current_token() const noexcept { return _current_token; }
  inline xml_token_type last_token() const noexcept { return _last_token; }

private:
  struct helper;

  zb::utf8_span_stream _stream;
  xml_token_type _current_token = tok_none;
  xml_token_type _last_token = tok_none;
  ...
  inline void set_token(xml_token_type t) { _last_token = std::exchange(_current_token, t); }
