...
ZS_JIT_COMPILER_PARSE_OP(p_function_statement) {
  ...
  if (is(tok_lsqrbracket)) {
    ZS_RETURN_IF_ERROR(parse<ps::p_bind_env>(REF(bound_target)));
  }
...
ZS_JIT_COMPILER_PARSE_OP(p_function, bool lambda) {
  ...
  if (is(tok_lsqrbracket)) {
    ZS_RETURN_IF_ERROR(parse<parse_op::p_bind_env>(REF(bound_target)));
  }

  ZS_RETURN_IF_ERROR(expect(tok_lbracket));

  object dummy;
  ZS_RETURN_IF_ERROR(parse<parse_op::p_create_function>(CREF(dummy), bound_target, lambda));
...
ZS_JIT_COMPILER_PARSE_OP(p_function_call_args, bool rawcall) {
  ...
  while (is_not(tok_rbracket)) {
    ...
    if (is(tok_comma)) {
      ...
      if (is(tok_rbracket)) {
        ...
        return handle_error(
            zs::error_code::invalid_token, "expression expected, found ')'", ZB_CURRENT_SOURCE_LOCATION());
      }
    }
  }
  lex();
  ...
  if (rawcall) {
    ...
    if (nargs < 3) {
      ...
      return handle_error(zs::error_code::invalid_argument,
  ...
  while (is_not(tok_rbracket)) {
    if (is(tok_triple_dots)) {
      ...
      if (is_not(tok_rbracket)) {
        return handle_error(zs::error_code::invalid_token, "expected ')' after a variadic (...) parameter",
            ZB_CURRENT_SOURCE_LOCATION());
      }
      ...
      if (has_type) {
        ...
        case tok_var:
          ...
          if (is(tok_lt)) {
            if (auto err = parse<parse_op::p_variable_type_restriction>(REF(mask), REF(custom_mask))) {
              return handle_error(
                  err, "parsing variable type restriction `var<....>`", ZB_CURRENT_SOURCE_LOCATION());
            }
          }
          break;
        }
      ...
      // to have one too.
        ...
        return handle_error(zs::error_code::invalid_token, "expected '=' after a default paramter definition",
            ZB_CURRENT_SOURCE_LOCATION());
      }
