...
static inline uint128_t umul256(
...
static inline void append_n_digits(const uint32_t olength, uint32_t digits, char* const result) {
  ...
  while (digits >= 10000) {
    ...
    memcpy(result + olength - i - 2, __zb::k_digit_table.data() + c0, 2);
    memcpy(result + olength - i - 4, __zb::k_digit_table.data() + c1, 2);
    i += 4;
  }
  if (digits >= 100) {
    const uint32_t c = (digits % 100) << 1;
    digits /= 100;
    memcpy(result + olength - i - 2, __zb::k_digit_table.data() + c, 2);
    i += 2;
  }
  if (digits >= 10) {
...
static inline void append_d_digits(const uint32_t olength, uint32_t digits, char* const result) {
  ...
  while (digits >= 10000) {
    ...
    const uint32_t c0 = (c % 100) << 1;
    const uint32_t c1 = (c / 100) << 1;
    memcpy(result + olength + 1 - i - 2, __zb::k_digit_table.data() + c0, 2);
    memcpy(result + olength + 1 - i - 4, __zb::k_digit_table.data() + c1, 2);
    i += 4;
  }
  if (digits >= 100) {
    const uint32_t c = (digits % 100) << 1;
    digits /= 100;
    memcpy(result + olength + 1 - i - 2, __zb::k_digit_table.data() + c, 2);
    i += 2;
  }
  if (digits >= 10) {
    const uint32_t c = digits << 1;
    result[2] = __zb::k_digit_table[c + 1];
    result[1] = '.';
    result[0] = __zb::k_digit_table[c];
  }
  else {
    result[1] = '.';
    result[0] = (char)('0' + digits);
...
static inline void append_c_digits(const uint32_t count, uint32_t digits, char* const result) {
  ...
  for (; i < count - 1; i += 2) {
    const uint32_t c = (digits % 100) << 1;
    digits /= 100;
    memcpy(result + count - i - 2, __zb::k_digit_table.data() + c, 2);
  }
...
static inline void append_nine_digits(uint32_t digits, char* const result) {
  ...
  for (uint32_t i = 0; i < 5; i += 4) {
    ...
    const uint32_t c0 = (c % 100) << 1;
    const uint32_t c1 = (c / 100) << 1;
    memcpy(result + 7 - i, __zb::k_digit_table.data() + c0, 2);
    memcpy(result + 5 - i, __zb::k_digit_table.data() + c1, 2);
  }
  result[0] = (char)('0' + digits);
}
...
static inline uint32_t pow10BitsForIndex(const uint32_t idx) { return 16 * idx + POW10_ADDITIONAL_BITS; }
...
int d2exp_buffered_n(double d, uint32_t precision, char* result, size_t sz) {
  ...
  if (exp >= 100) {
    ...
    memcpy(result + index, __zb::k_digit_table.data() + 2 * (exp / 10), 2);
    result[index + 2] = (char)('0' + c);
    index += 3;
  }
  else {
    memcpy(result + index, __zb::k_digit_table.data() + 2 * exp, 2);
    index += 2;
  }
