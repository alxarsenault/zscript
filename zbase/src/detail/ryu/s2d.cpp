...
static inline uint32_t floor_log2(const uint64_t value) { return 63 - __builtin_clzll(value); }
...
enum Status s2d_n(const char* buffer, size_t len, double* result) {
  ...
  uint64_t ieee
      = (((((uint64_t)signedM) << DOUBLE_EXPONENT_BITS) | (uint64_t)ieee_e2) << DOUBLE_MANTISSA_BITS)
      | ieee_m2;
  *result = int64Bits2Double(ieee);
  return SUCCESS;
}

enum Status s2d(const char* buffer, double* result) { return s2d_n(buffer, (int)strlen(buffer), result); }
